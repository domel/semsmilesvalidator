...
int main(int argc, char **argv)
{
    ...
    try {
        nt::Parser file = nt::Parser("./readme.nt");
        int c = (int)file.lineCount();
        ofstream myfile ("mol.smi");
    ...
    while (std::getline(ifs, line)) {
        ...
        try {
            ...
            if (e.type() == Exception::SemanticsError)
              std::cerr << "Error: " << e.what() << "." << std::endl;
              std::cerr << "Line number: " << no << " " << line << std::endl;
