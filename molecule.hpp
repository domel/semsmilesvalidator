...
struct Molecule {
    const Bond *bond(int source, int target) const {
        for (std::size_t i = 0; i < bonds.size(); ++i)
            if ((source == bonds[i].source && target == bonds[i].target) ||
                    (source == bonds[i].target && target == bonds[i].source))
                return &bonds[i];
        return 0;
    }
...
struct MoleculeSmilesCallback : public CallbackBase {
    ...
    void addAtom(int atomicNumber, bool aromatic, int isotope, int hCount,
                 int charge, int atomClass) {
    ...
    void setChiral(int index, Chirality chirality,
                   const std::vector<int> &chiralRefs) {
